...
export default async function captureOrderPayments(context, input = {}) {
  ...
  const capturePromises = orderPaymentsToCapture.map(async (payment) => {
    ...
    try {
      console.log("capturePayment", payment);
      result = await context.queries.getPaymentMethodConfigByName(payment.name).functions.capturePayment(context, {payment,orderId});
